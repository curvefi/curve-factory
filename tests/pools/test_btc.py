...
def test_btc_swaps(alice, bob, fee_receiver, swap_btc, factory, coin):
    ...
    underlying_coins = [coin] + [
        MintableForkToken(i) for i in factory.get_underlying_coins(swap_btc)[1:4]
    ]
    ...
    for idx in range(2):
        swap_btc.remove_liquidity_one_coin(
            10 ** wrapped_coins[idx].decimals(), idx, 0, {"from": alice}
        )
        chain.sleep(3600)
...
def test_zap_gas(DepositZapBTC, alice, bob, swap_btc, factory, coin):
    ...
    for idx in range(4):
        amounts = [10 ** i.decimals() for i in underlying_coins]
        amounts[idx] = 0
        zap.remove_liquidity_imbalance(
            swap_btc, amounts, swap_btc.balanceOf(alice), {"from": alice}
        )
        chain.sleep(3600)

    for idx in range(4):
        zap.remove_liquidity_one_coin(
            swap_btc, 10 ** underlying_coins[idx].decimals(), idx, 0, {"from": alice}
        )
        chain.sleep(3600)
