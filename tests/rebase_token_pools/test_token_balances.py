...
def test_get_balances(
    alice,
    mint_and_deposit,
    wrapped_rebase_coins,
    wrapped_rebase_decimals,
    wrapped_rebase_amounts,
    base_pool,
    swap_rebase,
):
...
def test_admin_balances_do_not_change(
    alice, swap_rebase, wrapped_rebase_coins, wrapped_rebase_amounts
):
...
def test_balances(
    alice, mint_and_deposit, swap_rebase, wrapped_rebase_coins, wrapped_rebase_amounts
):
    for i in range(len(wrapped_rebase_coins)):
        assert swap_rebase.balances(i) == wrapped_rebase_amounts[i]


def test_virtual_price_increases_with_balances(
    alice, mint_and_deposit, swap_rebase, wrapped_rebase_coins, wrapped_rebase_amounts
):
