...
def test_exchange(
    ...
    assert 1 - max(1e-4, 1/received) - 0.0004 < received / 10**wrapped_decimals[receiving] < 1 - 0.0004

    expected_admin_fee = 10**wrapped_decimals[receiving] * 0.0004 * 0.5
    admin_fee = swap.admin_balances(receiving)

    assert expected_admin_fee / admin_fee == approx(1, rel=max(1e-3, 1/(expected_admin_fee-1.1)))
