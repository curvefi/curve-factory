...
def _exchange(
    ...
    # If callback sig is empty, do transferFrom
    if callback_sig == empty(bytes32):
        ...
        raw_call(
            ...
            concat(
                ...
                _abi_encode(sender, receiver, coin, _dx, _min_dy)  # <-- Check! We're using min_dy here because dy is not known
            )
        )

    new_x: uint256 = ERC20(coin).balanceOf(self)
    dx: uint256 = ERC20(coin).balanceOf(self) - initial_x
...
def remove_liquidity(
    ...
    for i in range(N_COINS):
        ...
        assert ERC20(self.coins[i]).transfer(
            _receiver, value, default_return_value=True\
        )  # dev: failed transfer
