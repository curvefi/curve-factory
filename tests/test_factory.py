...
def test_find_pool_for_coins(factory, swap, wrapped_coins, sending, receiving):
    assert (
        factory.find_pool_for_coins(wrapped_coins[sending], wrapped_coins[receiving])
        == swap
    )
...
def test_find_pool_for_coins_underlying(factory, swap, underlying_coins, idx):
    assert (
        factory.find_pool_for_coins(underlying_coins[0], underlying_coins[idx]) == swap
    )
    assert (
        factory.find_pool_for_coins(underlying_coins[idx], underlying_coins[0]) == swap
    )


@pytest.mark.parametrize("sending,receiving", itertools.permutations(range(1, 4), 2))
def test_find_pool_underlying_base_pool_only(
    factory, underlying_coins, sending, receiving
):
    assert (
        factory.find_pool_for_coins(
            underlying_coins[sending], underlying_coins[receiving]
        )
        == ZERO_ADDRESS
    )
...
@pytest.mark.parametrize("sending,receiving", itertools.permutations(range(1, 4), 2))
def test_get_coin_indices_underlying(
    factory, swap, sending, receiving, underlying_coins
):
...
def test_deploy_metapool(MetaImplementationUSD, factory, base_pool, alice, bob):
    ...
    tx = factory.deploy_metapool(
        base_pool, "Name", "SYM", coin, 12345, 50000000, {"from": bob}
    )
